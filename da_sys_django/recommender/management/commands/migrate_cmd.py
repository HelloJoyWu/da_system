...
class Command(BaseCommand):
    ...
    Expects a secondary database configured with the name 'target'.
    Removes any conflicting models, and then copies models from the
    'default' database to the configured 'target' database.
    Remember to build "target" connection.
    """
    ...
    recommender_models = [SetupLog]

    # by default, migrate everything
    models_to_migrate = common_models + allauth_models + recommender_models
    ...
    def handle(self, *args, **options):
        ...
        if options['no_auth']:
            ...
            self.models_to_migrate += self.recommender_models
